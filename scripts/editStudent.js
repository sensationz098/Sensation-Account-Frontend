...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document.getElementById("editCourseStartDate").value =
    formatDate(studentData.courseStartDate) || "";
  document.getElementById("editCourseEndDate").value =
    formatDate(studentData.courseEndDate) || "";
  document.getElementById("editDateOfPayment").value =
    formatDate(studentData.date_of_payment) || "";
  ...
  try {
    ...
    if (studentData.course && courseAvailable) {
      const selectedOption = editCourseSelect.querySelector(
    ...
    if (studentData.timing && timingAvailable) {
      const selectedOption = editTimingSelect.querySelector(
        `option[value="${studentData.timing}"]`
      );
      if (selectedOption) {
        selectedOption.selected = true;
      }
    ...
    if (studentData.state && stateAvailable) {
      const selectedOption = editStateSelect.querySelector(
        `option[value="${studentData.state}"]`
      );
      if (selectedOption) {
        selectedOption.selected = true;
      }
  ...
  try {
    ...
    if (studentData.Teacher && teacherAvailable) {
      const selectedOption = teacherSelect.querySelector(
        `option[value="${studentData.Teacher}"]`
      );
      if (selectedOption) {
        selectedOption.selected = true;
      }
  ...
  try {
    ...
    all_users.forEach((user) => {
      ...
      if (
        studentData.assignedUserId &&
        user.username === studentData.assignedUserId
      ) {
        user_available = true;
      }
    });
    ...
    if (studentData.assignedUserId && user_available) {
      const selectedOption = userSelect.querySelector(
        `option[value="${studentData.assignedUserId}"]`
      );
      if (selectedOption) {
        selectedOption.selected = true;
      }
    ...
    .addEventListener("submit", async function (event) {
      ...
      try {
        const response = await fetch(
          `${SERVER_URL}/user/student/edit/${studentId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
