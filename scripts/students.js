...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadBatch() {
    try {
      ...
      data.forEach((batch) => {
        const option = document.createElement("option");
        option.value = batch.BatchName; // store name
  ...
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      try {
        const response = await fetch(`${SERVER_URL}/teachers/add`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: token,
  ...
  document
    ...
    .addEventListener("submit", handleAddCourseFormSubmit);
  ...
  document
    .getElementById("addStudentCloseBtn")
...
function calculateEndDate() {
  ...
  const endDate = new Date(
    ...
    startDate.getMonth() + durationInMonths,
    startDate.getDate()
  );
...
document
  .getElementById("courseStartDate2")
  .addEventListener("change", calculateEndDate);
document
  .getElementById("CourseDuration")
  .addEventListener("input", calculateEndDate);
...
async function addStudent() {
  ...
  try {
    const response = await fetch(`${SERVER_URL}/user/student/add`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
...
async function fetchStudents(
  ...
  let queryParams = `${SERVER_URL}/user/displaydownload?startDate=${startDate}&endDate=${endDate}&usernames=${selectedUserIds.join(
    ","
  )}&courseStart=${courseStartDate}&courseEnd=${courseEndDate}&creationDate=${creationDate}&PaymentDate=${PaymentDate}&coursename=${courseName}&fees=${courseFee}&contact=${contact}&teacher=${teacher}&timing=${timing}&receipt=${Receipt}`;
  ...
  try {
    ...
    students.forEach((student) => {
      if (
        ...
        student.previousCourses.length > maxPreviousCourses
      ) {
    ...
    const data = students.map((student) => {
      ...
      student.previousCourses.forEach((prevCourse, index) => {
        ...
        row[`prevCourse${index + 1}Payment`] =
          prevCourse.date_of_payment || "NA";
        row[`prevCourse${index + 1}Receipt`] = prevCourse.NewReceipt || "NA";
      });

      for (
        let i = student.previousCourses.length;
        i < maxPreviousCourses;
        i++
      ) {
...
async function fetchLatestReceipt() {
  try {
    const response = await fetch(`${SERVER_URL}/user/students/latest-receipt`);
    if (response.ok) {
      const data = await response.json();
...
async function handleExtendCourseButtonClick(studentId) {
  try {
    ...
    clearExtendModel();
    const response = await fetch(`${SERVER_URL}/user/student/${studentId}`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
      },
    ...
    const extendCourseModal = new bootstrap.Modal(
      document.getElementById("extendCourseModal")
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      extendCourseBtn.onclick = async () => {
        ...
        try {
          const updatedStudent = await extendCourse(
            studentId,
            additionalMonths,
            amount,
            date_of_payment,
...
async function extendCourse(
  ...
  try {
    const response = await fetch(
      `${SERVER_URL}/user/student/extend-course/${studentId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
function displayStudents(students, download) {
  ...
  if (students.length > 0) {
    ...
    students.forEach((student) => {
      if (
        student.previousCourses &&
        student.previousCourses.length > maxPreviousCoursesCount
      ) {
    ...
    students.forEach((student) => {
      ...
      headers.forEach((header) => {
        ...
        if (
          ...
          header === "courseStartDate" ||
          header === "courseEndDate" ||
          header === "createdAt"
        ) {
          td.textContent = formatDate2(student[header]);
        } else {
          td.textContent =
            student[header] !== undefined ? student[header] : "NA";
        }
        tr.appendChild(td);
        ...
        if (header === "contact") {
          if (header === "contact") {
            ...
            contactLink.addEventListener("click", () =>
              handleEditStudent(student, student.assignedUserName)
            );
      ...
      nameLink.addEventListener("click", () =>
        handleExtendCourseButtonClick(student._id)
      );
      ...
      extendCourseButton.addEventListener("click", () =>
        handleExtendCourseButtonClick(student._id)
      );
      ...
      deleteButton.addEventListener("click", () =>
        handleDeleteButtonClick(student._id)
      );
...
async function handleEditStudent(student) {
  try {
    ...
    console.error(error);
    alert(
      "An error occurred while loading student data. Please try again later."
    );
  }
}
...
function handleDeleteButtonClick(studentId) {
  if (confirm("Are you sure you want to delete this student?")) {
    fetch(`${SERVER_URL}/user/student/delete/${studentId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
