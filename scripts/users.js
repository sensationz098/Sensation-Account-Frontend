...
document.addEventListener("DOMContentLoaded", function () {
  const apiUrl = `${SERVER_URL}/user/allusers`;
...
function changePass(userId) {
  ...
  changePasswordForm.addEventListener("submit", function (event) {
    ...
    fetch(`${SERVER_URL}/user/changePassword/${userId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
...
function editProfile(userId) {
  ...
  if (currentUserId) {
    fetch(`${SERVER_URL}/user/allusers?id=${currentUserId}`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
      },
      ...
      .then((userData) => {
        ...
        document.getElementById("newUsername").value = userData[0].username;
  ...
  editProfileForm.addEventListener("submit", function (event) {
    ...
    fetch(`${SERVER_URL}/user/profile/update/${currentUserId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
