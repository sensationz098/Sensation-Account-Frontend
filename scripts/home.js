...
async function fetchLatestReceipt() {
  try {
    const resp = await fetch(`${SERVER_URL}/user/students/latest-receipt`);
    if (resp.ok) {
...
async function fetchDataAndDisplay(download = false) {
  try {
    ...
    const url = `${SERVER_URL}/user/students/testing?startDate=${formattedStartDate}&endDate=${formattedEndDate}`;
...
function displayDataInTable(students, download) {
  ...
  if (students.length > 0) {
    ...
    const headers = Object.keys(students[0]).filter(
      (header) =>
        header !== "_id" && header !== "__v" && header !== "previousCourses"
    );
...
function formatDateRangeForDisplay(selectedDates) {
  try {
    if (selectedDates.length === 1) {
      return `Selected Dates: ${selectedDates[0].toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
      })}`;
...
function formatDateRange(course) {
  ...
  const formattedStartDate = new Date(course.start).toLocaleDateString(
    "en-US",
    { year: "numeric", month: "short", day: "numeric" }
  );
  const formattedEndDate = new Date(course.end).toLocaleDateString("en-US", {
    year: "numeric",
    month: "short",
    day: "numeric",
  });
...
function triggerDownload() {
  try {
    ...
    const ws = XLSX.utils.table_to_sheet(
      document.getElementById("tabularData")
    );
